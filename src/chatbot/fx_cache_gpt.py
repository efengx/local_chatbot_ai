...
def _loads_generations(generations_str: str) -> Union[RETURN_VAL_TYPE, None]:
    ...
    try:
        ...
        generations = [Generation(**generation_dict)
                       for generation_dict in gen_dicts]
        logger.warning(
...
class SQLAlchemyCache(BaseCache):
    ...
    def update(self, prompt: str, llm_string: str, return_val: RETURN_VAL_TYPE) -> None:
        ...
        items = [
            self.cache_schema(prompt=prompt, llm=llm_string,
                              response=dumps(gen), idx=i)
...
class FxGPTCache(BaseCache):
    ...
    def lookup(self, prompt: str, llm_string: str) -> Optional[RETURN_VAL_TYPE]:
        ...
        if res:
            return [
                ChatGeneration(**generation_dict) for generation_dict in json.loads(res)
            ]
        return None
    ...
    def update(self, prompt: str, llm_string: str, return_val: RETURN_VAL_TYPE) -> None:
        ...
        _gptcache = self._get_gptcache(llm_string)
        handled_data = json.dumps([generation.dict()
...
class MomentoCache(BaseCache):
    ...
    def __init__(
        ...
        *,
        ...
        if not isinstance(cache_client, CacheClient):
            raise TypeError(
                "cache_client must be a momento.CacheClient object.")
        _validate_ttl(ttl)
    ...
    def from_client_params(
        ...
        **kwargs: Any,
        ...
        try:
            api_key = auth_token or get_from_env(
                "auth_token", "MOMENTO_AUTH_TOKEN")
        except ValueError:
    ...
    def update(self, prompt: str, llm_string: str, return_val: RETURN_VAL_TYPE) -> None:
        ...
        set_response = self.cache_client.set(
            self.cache_name, key, value, self.ttl)
